...
export class Search extends BaseActivity {
    ...
    private async runSearchSession(page: Page, isMobile: boolean, tracker: SearchTracker): Promise<SessionStats> {
        ...
        try {
            ...
            // 审计：会话开始时强制落一条 Cookie 基线（来源/关键字段/指纹/归属）
            await this.bot.browser.func.captureSearchCookieSnapshot('SEARCH-COOKIE-SEED', URLs.bing.origin, 'primary', true)
            // 弱设备（RK3229 / armv7l）上等 load 事件常因挂起的第三方资源卡满 90s 导航超时，
            // 导致整个搜索会话在开场就异常退出（搜索次数=0）。只等 domcontentloaded，
            // 搜索框是否可用由下面 bingSearch() 里的 waitFor 保证。
            await page.goto(URLs.bing.origin, { waitUntil: 'domcontentloaded', timeout: 60000 })
            ...
            while (!tracker.done() && stats.performed < tracker.maxSearches && stats.stagnant < tracker.stagnantLimit) {
                ...
                if (gained > 0) {
                    ...
                    // 审计：未获得积分时，抓取当时的 Cookie 状态并给出"是否 Cookie 导致"的结论
                    const zeroSnapshot = await this.bot.browser.func.captureSearchCookieSnapshot(
                        'SEARCH-ZERO-POINTS',
                        URLs.bing.origin,
                        'primary',
                        true
                    ...
                    this.bot.logger.info(
                        isMobile,
                        'SEARCH-COOKIE-AUDIT',
                        `未获得积分原因排查 | ${this.bot.browser.func.explainZeroPoints(zeroSnapshot)}`
                    )
                }
            }
